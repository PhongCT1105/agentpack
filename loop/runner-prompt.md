...
**1. Pick work.** Take the FIRST unchecked task (`- [ ]`) in `docs/backlog.md`. After a task lands fully (green, reviewed, committed, pushed), you may continue to the next unchecked task in the same session; adjacent tasks marked `[indep]` may additionally run as a parallel subagent batch (up to 3 at once). Hand off — exit the session — when you reach a phase boundary, or when your context is getting heavy enough that quality would suffer: fresh sessions are cheap, half-attention is not. Never reorder, never cherry-pick deeper into the list, never touch tasks marked `[!]` (blocked), and never exit with a task half-done (finish it or revert it).
