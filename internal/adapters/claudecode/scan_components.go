...
// scanAgents reads agent definitions (<root>/agents/*.md). The frontmatter
// name overrides the filename stem, matching Claude Code.
func (a *Adapter) scanAgents(inv *model.Inventory, root string, scope model.Scope) error {
	return mdscan.ScanFlatDir(inv, filepath.Join(root, "agents"), true,
...
func (a *Adapter) scanCommands(inv *model.Inventory, root string, scope model.Scope) error {
	return mdscan.ScanFlatDir(inv, filepath.Join(root, "commands"), false,
		func(name, path, description string) {
