...
- **Topology: one worker session per iteration, multi-agent inside.** The worker may batch up to 3 adjacent `[indep]` backlog tasks and dispatch parallel implementer subagents (git worktrees for isolation), plus a reviewer subagent over the final diff. Commits land serially from one process — no cross-process git races.
